...
int main(){
    double n_u = 1000.; // number of x values used for u(x)
    arma::vec x_u = arma::linspace(0, 1, n_u); //create array with x values
    arma::vec func = arma::vec(n_u); //define array for function values of u(x)
    for (int i = 0; i < n_u; i++){ // compute u(x) for each x value
      func(i) = u(x_u(i));
    }
    ...
    double n = 10.; // number of x values used for v
